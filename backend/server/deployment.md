...
export GEMINI_API_KEY=AIza...   # optional; stubs used if unset
uvicorn app:app --reload --host 0.0.0.0 --port 8000
```

API: `http://localhost:8000`. Health: `GET /health`. Chat: `POST /api/chat`.

**RAG index (one-time):** with `GEMINI_API_KEY` set, from `backend/` run `python scripts/build_rag.py` to embed `database/questionnaire.csv` + `database/output.json` into `rag/faiss_index/`.
...
- `GEMINI_API_KEY` — direct LLM + embeddings when Langflow flow IDs are not set for an agent.
