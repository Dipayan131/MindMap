...
Python code lives under repo root **`backend/`** (run `uvicorn` with `cd backend`).

The **`frontend/`** Next app calls `POST /api/chat` and `POST /api/user/questionnaire` against `NEXT_PUBLIC_API_URL` (default `http://127.0.0.1:8000`). FastAPI enables **CORS** for local Next (`CORS_ORIGINS` in backend config).
